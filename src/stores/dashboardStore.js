...
export const useDashboardStore = defineStore('dashboard', () => {
  ...
  const calculatedCost = computed(() => {
    const pStat = providerStats.value.find(p => p.name === costCalc.value.provider)
    if (!pStat) return 0
    return (pStat.promptTokens / 1000000) * costCalc.value.inputPrice +
  ...
  async function fetchHistoricalStats() {
    try {
      const res = await configAPI.request(`/api/admin/dashboard/stats?range=${timeRange.value}`)
      if (res.success) {
        const data = res.data
        ...
        providerStats.value = data.modelDistribution.reduce((acc, curr) => {
          ...
          if (!existing) {
            existing = { name: provName, hitTokens: 0, missTokens: 0, calls: 0, promptTokens: 0, compTokens: 0 }
            acc.push(existing)
          }
          ...
          existing.missTokens += curr.cacheMissTokens || (curr.promptTokens + curr.candidatesTokens - (curr.cacheHitTokens || 0))

          const total = existing.hitTokens + existing.missTokens
          existing.cacheHitRate = total > 0 ? Math.round((existing.hitTokens / total) * 100) : 0
          return acc
        }, [])
        ...
        groupedProviders.value = Object.entries(providerGroups).map(([name, totalTokens]) => ({
          name,
          totalTokens
  ...
  async function fetchFailures() {
    try {
      const res = await configAPI.request('/api/admin/dashboard/failures?limit=50&offset=0')
      if (res.success) {
  ...
  async function fetchTurns() {
    try {
      const res = await configAPI.request('/api/admin/dashboard/turns?limit=50&offset=0')
      if (res.success) {
  ...
  async function selectTurn(turn) {
    ...
    try {
      const res = await configAPI.request(`/api/admin/dashboard/trace/${turn.requestId}`)
      if (res.success) {
