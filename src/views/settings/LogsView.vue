  ...
  <div class="logs-view">
    ...
    <el-card class="connection-card" shadow="never">
      <div class="connection-status">
        ...
        <div class="connection-info">
          <span v-if="isConnected && stats">
            服务运行时间: {{ formatUptime(stats.service?.uptime) }} | 总日志数:
            {{ stats.service?.totalLogsSent || 0 }}
          </span>
          <span v-else-if="isConnected"> 连接正常，正在加载日志... </span>
          <span v-else> 请检查网络连接和服务器状态 </span>
        </div>
      </div>
      ...
      >
        ...
        <div>
          logStore是否初始化: {{ logStore.isInitialized ? "是" : "否" }}
        </div>
      </div>
    </el-card>
      ...
      <div ref="logsContainer" class="logs-list" @scroll="handleScroll">
        ...
        >
          ...
          <div class="log-message">
            <span
              v-if="log.extra?.type === 'json'"
              class="json-message"
              @click="openJsonDialog(log)"
            >
              {{ getJsonSummary(log) }}
              <el-icon class="json-expand-icon"><View /></el-icon>
            </span>
            <span
              v-else-if="isObjectMessage(log.message)"
              class="object-message"
              @click="openObjectDialog(log)"
            >
          ...
          <div
            v-if="
              log.extra && Object.keys(getDisplayExtra(log.extra)).length > 0
            "
            class="log-extra"
          >
            <el-tag
              ...
              @click="
                isObjectValue(value)
                  ? openExtraObjectDialog(key, value, log)
                  : null
              "
            >
    ...
    >
      <div class="json-content">
        <div class="json-header">
          ...
          <span
            class="json-level"
            :class="`level-${selectedLog?.level?.toLowerCase()}`"
          >
            {{ selectedLog?.level }}
          </span>
