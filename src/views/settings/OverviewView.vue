      ...
      <div v-if="!loading" class="content-container">
          ...
          <el-card class="stat-card">
            ...
            <div class="stat-content">
              <div class="stat-value">
                {{ configStore.enabledAdaptersCount }}
              </div>
        ...
        <el-card class="section-card">
          ...
          <div class="adapters-overview">
            <div
              v-if="activeAdapterTypes.length === 0"
              class="empty-state"
              style="padding: 32px 0"
            >
              <el-empty
                description="暂无任何适配器实例"
                :image-size="100"
              ></el-empty>
            </div>
            <div
...
const allAdapterTypes = computed(() => {
  ...
  const configOnlyTypes = typesFromConfig.filter(
    (type) => !typesFromApi.includes(type),
  );
...
const activeAdapterTypes = computed(() => {
  return allAdapterTypes.value.filter(
    (type) => getAdapterInstanceCount(type) > 0,
  );
});
...
// 平板端适配器列表优化 @media (min-width: 769px) and (max-width: 1024px) {
.adapters-overview { .adapter-item { .adapter-name { max-width: 120px; }
.adapter-model { max-width: 100px; } } } } // 超小屏幕适配 @media (max-width:
480px) { .adapters-overview { .adapter-item { padding: 8px; gap: 6px; .el-tag {
min-width: 45px; font-size: 10px; padding: 2px 6px; } .adapter-name { max-width:
80px; font-size: 13px; } .adapter-model { max-width: 60px; font-size: 11px; } }
} }
