  ...
  <div class="image-adapters-view">
    ...
    <div class="page-header">
      <div class="header-title">
        ...
        <div class="header-desc">
          配置生图厂商（Google Gemini Nano Banana、OpenAI、硅基流动
          FLUX、火山引擎、土块绘图、SD WebUI 等），支持 AnyUI 异步渲染
        </div>
      </div>
      ...
      <el-table :data="adapters" v-loading="loading" style="width: 100%">
        ...
        <el-table-column prop="adapterType" label="类型/厂商" width="190">
          <template #default="{ row }">
            <el-tag :type="getTypeTagType(row.adapterType)">{{
              getTypeName(row.adapterType)
            }}</el-tag>
          </template>
        </el-table-column>
    ...
    >
      <el-form :model="form" label-width="140px" class="adapter-config-form">
        <el-form-item label="实例名称" required>
          <el-input
            ...
            placeholder="例如: Google-NanoBanana-2"
          />
        </el-form-item>
              ...
              <el-form-item v-if="field.secret" :required="field.required">
                <template #label>
                  <div class="form-item-label">
                    ...
                    <el-tooltip
                      v-if="field.tip || field.description"
                      :content="field.tip || field.description"
                      placement="top"
                    >
                      <el-icon class="help-tooltip-icon"
                        ><QuestionFilled
                      /></el-icon>
                    </el-tooltip>
                  </div>
                ...
                <el-input
                  ...
                  show-password
                  :placeholder="
                    field.placeholder || field.default || '请输入密钥'
                  "
                />
              </el-form-item>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="saving" @click="saveAdapter"
          >保存</el-button
        >
      </template>
    </el-dialog>
    ...
    >
      <el-form label-width="110px">
        <el-form-item label="提示词" required>
          <el-input
            v-model="testPrompt"
            placeholder="例如: A serene Japanese garden with cherry blossoms, digital art 8k"
            @keyup.enter="runTest"
          />
        </el-form-item>
        <el-form-item label="参考图(可选)">
          <el-input
            v-model="testImage"
            placeholder="支持输入图片 URL 或 Base64 (图生图模式)"
            clearable
          />
        </el-form-item>
        <el-form-item label="指定通道">
          <el-select
            v-model="testAdapterId"
            placeholder="默认使用系统默认生图实例"
            style="width: 100%"
            clearable
          >
            <el-option
              v-for="a in adapters"
              :key="a.id"
              :label="`${a.instanceName} (${a.adapterType})`"
      ...
      <div v-if="testResult" class="test-results-list">
        ...
        <div class="search-result-item">
          <img
            v-if="testResult.url || testResult.base64"
            :src="testResult.url || testResult.base64"
            alt="Generated image"
            class="preview-image"
          />
          <a
            ...
            >查看高清源图地址</a
          >
          <p v-if="testResult.revisedPrompt" class="result-snippet">
            优化提示词: {{ testResult.revisedPrompt }}
          </p>
        </div>
      </div>
...
const defaultTypes = [
  {
    ...
    name: "Google Gemini",
    description:
      "Google 原生图像生成（支持 AI Studio 与企业级 Vertex AI 双通道）",
    configSchema: {
      platform: {
        type: "string",
        label: "接入平台",
  ...
  },
  {
    ...
    configSchema: {
      ...
      baseUrl: {
        type: "string",
        label: "API Base URL",
        default: "http://datukuai.top:1450",
      },
      sampler: {
        type: "string",
        label: "采样器 (Sampler)",
        default: "Euler a",
        options: ["Euler a", "Euler", "DPM++ 2M Karras", "DDIM"],
      },
      ...
      negativePrompt: {
        ...
        label: "默认负向词",
        default:
          "(easynegative:1.1), (verybadimagenegative_v1.3:1), (low quality:1.2), (worst quality:1.2)",
      },
    },
  },
  {
...
const defaultAdapterName = computed(() => {
  ...
  return def
    ? `${def.instanceName} (${getTypeName(def.adapterType)})`
    : "未指定";
});
...
const testPrompt = ref(
  "A serene Japanese garden with cherry blossoms, digital art 8k",
);
const testImage = ref("");
...
const currentSchema = computed(() => {
  const meta = allAvailableTypes.value.find(
    (t) => t.type === form.value.adapterType,
  );
...
const fetchAdapters = async () => {
  ...
  try {
    ...
    ElMessage.error(
      "加载生图适配器失败: " + (err.message || "请先验证管理员访问码"),
    );
  } finally {
    loading.value = false;
  }
...
const saveAdapter = async () => {
  ...
  try {
    const url = isEdit.value
      ? `/api/images/adapters/${currentId.value}`
      : "/api/images/adapters";
