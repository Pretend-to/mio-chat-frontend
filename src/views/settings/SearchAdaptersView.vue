  ...
  <div class="search-adapters-view">
    ...
    <div class="page-header">
      <div class="header-title">
        ...
        <div class="header-desc">
          配置与管理 LLM 联网搜索通道（如 Tavily
          等），并配合系统内置兜底引擎实现高可用联网检索
        </div>
      </div>
    ...
    <el-card shadow="never" class="fallback-card">
      ...
      <div class="fallback-desc">
        系统已内置无需配置的轻量级搜索引擎。当未配置自定义搜索实例，或主搜索引擎调用失败（如超时、网络波动、Key
        配额耗尽）时，系统将按顺序自动无缝级联调用内置兜底引擎，确保 LLM
        联网检索始终可用。
      </div>
      <div class="fallback-chips">
        <div
          v-for="engine in fallbackEnginesList"
          :key="engine.type"
          class="fallback-chip"
        >
      ...
      <template #header>
        <div class="card-header-flex">
          ...
          <span class="card-subtitle"
            >（用于配置高级 API 搜索引擎，优先作为主通道使用）</span
          >
        </div>
      </template>
      <el-table
        ...
        v-loading="loading"
        style="width: 100%"
        empty-text="暂无自定义搜索实例，系统将默认使用内置兜底引擎"
      >
      ...
      <el-form :model="form" label-width="140px" class="adapter-config-form">
        <el-form-item label="实例名称" required>
          <el-input
            ...
            placeholder="例如: Tavily-Agent-Search"
          />
        </el-form-item>
  ...
  return [
    {
      type: "duckduckgo",
      name: "DuckDuckGo",
      description: "免注册、免 Key 的轻量化 HTML 搜索",
    },
    { type: "bing", name: "Bing", description: "微软必应搜索网页通道" },
    {
      type: "baidu",
      name: "百度搜索",
      description: "中文互联网高质量网页检索",
    },
  ];
});
...
const fetchAdapters = async () => {
  ...
  try {
    ...
    ElMessage.error(
      "加载搜索适配器失败: " + (err.message || "请先验证管理员访问码"),
    );
  } finally {
    loading.value = false;
  }
...
const saveAdapter = async () => {
  ...
  try {
    const url = isEdit.value
      ? `/api/search/adapters/${currentId.value}`
      : "/api/search/adapters";
