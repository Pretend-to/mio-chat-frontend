  ...
  <div class="vision-adapters-view">
    ...
    <div class="page-header">
      <div class="header-title">
        ...
        <div class="header-desc">
          复用「LLM 适配器」中已启用的多模态模型，支持纯文本推理模型跨通道中继与
          OCR 识别
        </div>
      </div>
    ...
    <div class="stats-bar">
      <div class="stat-item">
        ...
        <span class="stat-value">{{
          form.mode === "custom" ? "指定专属视觉模型" : "智能自动选择 (推荐)"
        }}</span>
      </div>
      <div class="stat-item">
    ...
    <el-card shadow="never" class="settings-card">
      <template #header>
        <div class="card-header-flex">
          ...
          <span class="card-subtitle"
            >（基于 LiteLLM 规范精准识别已配置的多模态模型，无需重复配置 API
            Key）</span
          >
        </div>
      </template>
      ...
      <el-form
        :model="form"
        label-width="140px"
        class="adapter-config-form"
        v-loading="loading"
      >
        <el-form-item label="调度模式">
          <el-radio-group v-model="form.mode">
            <el-radio value="auto"
              >智能自动选择（自动使用当前启用的首选多模态模型）</el-radio
            >
            <el-radio value="custom"
              >指定专属视觉通道（手动指定特定 LLM 渠道与模型）</el-radio
            >
          </el-radio-group>
        </el-form-item>
        ...
        >
          ...
          <div v-if="availableModels.length === 0" class="field-tip error">
            ⚠️ 当前系统中未检测到已启用的多模态视觉模型，请前往「LLM
            适配器」页面配置并启用具备 Vision 能力的模型（如
            Gemini、GPT-4o、Qwen-VL 等）。
          </div>
        </el-form-item>
    ...
    >
      <el-form label-width="90px">
        <el-form-item label="测试图片" required>
          <el-input
            ...
            placeholder="输入公网图片 URL 或本地绝对路径"
          />
        </el-form-item>
      ...
      <div v-if="testResult" class="test-result-box">
        <div class="result-meta">
          ...
          <span v-if="testResult.durationMs" class="duration-text"
            >耗时: {{ testResult.durationMs }}ms</span
          >
        </div>
...
const form = ref({
  ...
  model: "",
  defaultPrompt:
    "请详细描述这幅图片中的内容，提取主要主体、场景特征和文字信息。",
});
...
const activeModelDisplayName = computed(() => {
  if (form.value.mode === "custom" && form.value.provider && form.value.model) {
    const match = availableModels.value.find(
      (m) => m.provider === form.value.provider && m.model === form.value.model,
    );
