  ...
  >
    <div style="margin-bottom: 16px; display: flex; align-items: center; justify-content: space-between; background-color: #f8fafc; padding: 12px 16px; border-radius: 8px; border: 1px solid #e2e8f0; flex-wrap: wrap; gap: 8px;">
      <div style="font-size: 14px; color: #475569; font-weight: 500; display: flex; align-items: center; gap: 8px;">
        <span>图片尺寸：</span>
        <el-radio-group :model-value="exportWidthMode" size="small" @change="onWidthModeChange">
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button :disabled="generatingImage || !previewImageUrl" @click="$emit('copy')">复制到剪贴板</el-button>
        <el-button :disabled="generatingImage || !previewImageUrl" type="primary" @click="$emit('download')">下载图片</el-button>
      </span>
    </template>
  </el-dialog>
  ...
  >
    <div style="height: 100%; display: flex; flex-direction: column; background: #f2f2f2;">
      <div class="mobile-preview-header" style="display: flex; justify-content: space-between; align-items: center; padding: 15px 20px; background-color: #fff; border-bottom: 1px solid #ebeef5;">
        <span @click="onClose" style="font-size: 16px; color: #606266; cursor: pointer;"><i class="iconfont icon-return"></i> 返回</span>
      ...
      <div style="padding: 10px 20px; background-color: #fff; border-bottom: 1px solid #f1f5f9; display: flex; justify-content: center;">
        <el-radio-group :model-value="exportWidthMode" size="default" @change="onWidthModeChange">
      ...
      <div class="mobile-preview-footer" style="padding: 20px; background: transparent; border: none; display: flex; gap: 12px;">
        <el-button @click="$emit('share-link')" size="large" style="flex: 1; border-radius: 12px; font-weight: bold;">分享链接</el-button>
        <el-button :disabled="generatingImage || !previewImageUrl" type="primary" @click="$emit('download')" size="large" style="flex: 1; border-radius: 12px; font-weight: bold;">保存图片</el-button>
      </div>
    </div>
  </el-drawer>
</template>
