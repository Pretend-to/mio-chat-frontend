  ...
  <div class="overview-container">
    ...
    <div class="stats-grid">
      <div class="stat-card connections">
        <div class="stat-info">
          ...
          <span class="stat-value text-green">{{ store.stats.connections }}</span>
        </div>
      ...
      <div class="stat-card token-metrics">
        <div class="stat-info">
          ...
          <span class="stat-value text-indigo">{{ formatTokens(store.stats.totalTokens) }}</span>
          <span class="stat-sub">
            入: {{ formatTokens(store.stats.promptTokens) }} | 出: {{ formatTokens(store.stats.compTokens) }}
          </span>
        </div>
      ...
      <div class="chart-card-col col-8">
        <div class="saas-card">
          <div class="card-header">
            ...
            <el-select v-model="store.slaMetric" size="small" class="saas-select" style="width: 150px;">
    ...
    <div class="saas-card mt-lg">
      ...
      <div class="card-body p-none">
        <el-table :data="store.providerStats" style="width: 100%" class="saas-table">
          <el-table-column prop="name" label="Provider / 实例"></el-table-column>
          ...
          <el-table-column prop="hitTokens" label="命中 Token 数">
            <template #default="scope">{{ formatNumber(scope.row.hitTokens) }}</template>
          </el-table-column>
          <el-table-column prop="missTokens" label="未命中 Token 数">
            <template #default="scope">{{ formatNumber(scope.row.missTokens) }}</template>
          </el-table-column>
          <el-table-column prop="calls" label="总调用次数">
            <template #default="scope">{{ formatNumber(scope.row.calls) }}</template>
          </el-table-column>
        </el-table>
      </div>
    </div>
...
function renderCharts() {
  ...
  if (shareChart && store.historicalData.modelDistribution) {
    ...
    shareChart.setOption({
      ...
      color: ['#2563eb', '#10b981', '#6366f1', '#f59e0b', '#ec4899', '#94a3b8'],
      series: [{
        type: 'pie',
        radius: ['45%', '70%'],
