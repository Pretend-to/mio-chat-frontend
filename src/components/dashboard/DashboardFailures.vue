      ...
      <div class="right-col-8">
        <div class="saas-card table-card">
          ...
          <div class="card-body p-none">
            <el-table :data="store.failures" size="default" class="saas-table" height="340">
              <el-table-column label="发生时间" width="160">
                <template #default="scope">{{ formatTime(scope.row.createdAt) }}</template>
              </el-table-column>
              <el-table-column prop="requestId" label="请求 ID" min-width="140" show-overflow-tooltip>
              ...
              <el-table-column prop="model" label="模型" width="130" show-overflow-tooltip>
                <template #default="scope">
                  <el-tag size="small" type="danger" effect="plain">{{ scope.row.model }}</el-tag>
                </template>
              </el-table-column>
              <el-table-column prop="errorMessage" label="错误信息" min-width="200" show-overflow-tooltip></el-table-column>
              <el-table-column label="诊断" width="90" align="center" fixed="right">
                <template #default="scope">
                  <el-button size="small" type="danger" plain class="trace-btn" @click="showTrace(scope.row)">
                    Trace
                  </el-button>
                </template>
              </el-table-column>
...
function renderChart() {
  ...
  const errMap = store.failures.reduce((acc, curr) => {
    ...
    if (msg.includes('429') || msg.toLowerCase().includes('rate limit')) cat = 'Rate Limit (429)'
    else if (msg.includes('504') || msg.toLowerCase().includes('timeout')) cat = 'Network Timeout (504)'
    else if (msg.includes('401') || msg.toLowerCase().includes('auth')) cat = 'Auth Failure (401)'

    acc[cat] = (acc[cat] || 0) + 1
    return acc
  }, {})

  const chartData = Object.entries(errMap).map(([name, value]) => ({ name, value }))
  ...
  errorChart.setOption({
    ...
    series: [{
      type: 'pie',
      radius: '60%',
      data: chartData.length > 0 ? chartData : [{ name: '无故障运行', value: 0 }],
