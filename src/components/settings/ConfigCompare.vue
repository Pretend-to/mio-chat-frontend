          ...
          <div v-if="hasDifferences" class="diff-details">
            <el-tabs v-model="activeTab">
              <el-tab-pane label="LLM 适配器" name="adapters">
                <div
                  class="diff-section"
                  v-for="(changes, type) in adapterDiffs"
                  :key="type"
                >
...
const adapterDiffs = computed(() => {
  ...
  // 动态获取适配器类型
  const adapterTypes =
    configStore?.adapterTypes?.types ||
    Object.keys(currentConfig.value.adapters || {});
...
const serverDifferences = computed(() => {
  return getConfigDifferences(
    currentConfig.value.server,
    compareConfig.value?.server,
  );
});
...
const webDifferences = computed(() => {
  return getConfigDifferences(
    currentConfig.value.web,
    compareConfig.value?.web,
  );
});
...
const onebotDifferences = computed(() => {
  return getConfigDifferences(
    currentConfig.value.onebot,
    compareConfig.value?.onebot,
  );
});
